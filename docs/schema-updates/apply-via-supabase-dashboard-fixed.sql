-- Alert24 Schema Updates - For Supabase Dashboard Application (FIXED)
-- Migration: 12_missing_columns_and_fixes.sql (Dashboard Compatible - No CONCURRENTLY)
-- 
-- INSTRUCTIONS:
...
-- 5. Verify with: /api/test-migration-status
...
-- Add created_by column to on_call_schedules if missing
...
-- =============================================================================
-- 7. CREATE INDEXES FOR PERFORMANCE (WITHOUT CONCURRENTLY)
-- =============================================================================

-- Indexes for better query performance
CREATE INDEX IF NOT EXISTS idx_notification_rules_escalation_policy 
    ON public.notification_rules(escalation_policy_id);

CREATE INDEX IF NOT EXISTS idx_notification_history_incident 
...
CREATE INDEX IF NOT EXISTS idx_notification_history_organization 
...
CREATE INDEX IF NOT EXISTS idx_organization_members_invitation_status 
    ON public.organization_members(invitation_status);

CREATE INDEX IF NOT EXISTS idx_organization_members_invitation_email 
...
SELECT 
    'Migration 12_missing_columns_and_fixes.sql completed successfully via Supabase Dashboard' as status,
    NOW() as completed_at;