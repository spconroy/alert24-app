...
<body>
  ...
  async sendIncidentPaging({
    ...
    const severityConfig = {
      critical: { color: '#dc3545', icon: '🚨', priority: 'urgent' },
      high: { color: '#fd7e14', icon: '⚠️', priority: 'high' },
      medium: { color: '#ffc107', icon: '⚡', priority: 'high' },
      low: { color: '#28a745', icon: 'ℹ️', priority: 'normal' },
      maintenance: { color: '#6c757d', icon: '🔧', priority: 'normal' },
    };

    const config = severityConfig[severity] || severityConfig.medium;

    const subject = `🚨 URGENT: Incident Page - ${incidentTitle}`;
    
    const textContent = `
URGENT INCIDENT PAGE

${config.icon} SEVERITY: ${severity.toUpperCase()}

Incident: ${incidentTitle}
Description: ${incidentDescription}

This incident requires immediate attention. Please acknowledge by visiting:
${incidentUrl}

Incident ID: ${incidentId}
Time: ${new Date().toLocaleString()}

This is an automated page from Alert24.
    `.trim();
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>URGENT: Incident Page</title>
    <style>
        body { 
            margin: 0; 
            padding: 0; 
...
<body>
    <div class="container">
        <div class="urgent-header">
            <h1 style="margin: 0; font-size: 28px;" class="blinking">🚨 URGENT INCIDENT PAGE</h1>
            <p style="margin: 10px 0 0 0; opacity: 0.9; font-size: 18px;">${config.icon} ${severity.toUpperCase()} SEVERITY</p>
        </div>
        
        <div class="content">
            <h2 style="margin-top: 0; color: ${config.color};">${incidentTitle}</h2>
            ...
            <div style="text-align: center; margin: 30px 0;">
                <a href="${incidentUrl}" class="action-button" style="color: white;">
                    🚨 ACKNOWLEDGE INCIDENT NOW
                </a>
            </div>
            
            <p style="font-size: 16px; color: #e74c3c; font-weight: bold; text-align: center;">
                ⚠️ This incident requires immediate attention. Please acknowledge ASAP.
            </p>
            
            <p style="font-size: 14px; color: #666;">
                You are receiving this urgent page because you are assigned to this incident or are on-call. 
                Please visit the incident page to acknowledge and begin resolution.
            </p>
        </div>
        
        <div class="footer">
            <p>🚨 URGENT AUTOMATED PAGE FROM ALERT24</p>
            <p>Incident Management System - ${organizationBranding.name || 'Your Organization'}</p>
        </div>
    </div>
</body>
</html>
