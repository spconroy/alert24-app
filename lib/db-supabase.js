...
export class SupabaseClient {
  ...
  async getIncidentsByOrganization(organizationId) {
    ...
    // Get organization name separately to avoid join issues
    const organization = await this.getOrganizationById(organizationId);
    const organizationName = organization?.name;
    
    // Get all unique service IDs from incidents to resolve names
    const allServiceIds = new Set();
    (data || []).forEach(incident => {
      if (incident.affected_services) {
        incident.affected_services.forEach(service => {
          if (typeof service === 'string' && 
              service.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
    ...
    // Fetch service names if we have any IDs to resolve
    let serviceMap = {};
    if (allServiceIds.size > 0) {
      try {
        ...
        console.error('Error resolving service names for incident list:', serviceError);
      }
    }
    ...
    const transformedData = (data || []).map(incident => ({
      ...
      // Resolve service IDs to names in affected_services
      affected_services: incident.affected_services ? 
        incident.affected_services.map(service => {
          if (typeof service === 'string' && serviceMap[service]) {
  ...
  async createIncident(incidentData) {
    // Try to generate incident number per organization, but handle missing column gracefully
    let nextIncidentNumber = null;
    
    try {
      const { data: maxIncident, error: maxError } = await this.client
        .from('incidents')
      ...
      if (maxError) {
        console.warn('incident_number column not found, skipping numbering:', maxError.message);
      } else {
        nextIncidentNumber = (maxIncident?.incident_number || 0) + 1;
      }
    } catch (error) {
      console.warn('Could not fetch incident number, proceeding without it:', error.message);
    }

    // Only include incident_number if we successfully generated one
    const dataToInsert = nextIncidentNumber 
      ? { ...incidentData, incident_number: nextIncidentNumber }
      : incidentData;
  ...
  async getUserById(userId) {
    const { data, error } = await this.client
      .from('users')
      .select('id, name, email, avatar_url, phone_number, incident_role, is_on_call, created_at, updated_at')
      .eq('id', userId)
      .single();
  ...
  async getIncidentById(incidentId, userId) {
    ...
    // Resolve service IDs to service names in affected_services
    if (incident.affected_services && incident.affected_services.length > 0) {
      try {
        // Check if affected_services contains UUIDs (indicating they are service IDs)
        const possibleServiceIds = incident.affected_services.filter(service => 
          typeof service === 'string' && 
          service.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)
        );

        if (possibleServiceIds.length > 0) {
          // Fetch service names for these IDs
          const { data: services, error: servicesError } = await this.client
            .from('services')
            .select('id, name')
            .in('id', possibleServiceIds);
          ...
          if (!servicesError && services) {
            ...
            // Replace IDs with names in affected_services
            incident.affected_services = incident.affected_services.map(service => {
              if (typeof service === 'string' && serviceMap[service]) {
                return serviceMap[service];
              }
              return service;
        ...
        console.error('Error resolving service names:', serviceError);
        // Continue without resolving names - don't fail the whole request
      }
    }
  ...
  async getIncidentUpdates(incidentId) {
    // Use admin client to bypass RLS for retrieving incident updates
    ...
    console.log('📋 Raw incident updates:', { incidentId, data, error });

    if (error) throw error;

    // If we have updates, get user info separately
    if (data && data.length > 0) {
      const userIds = [...new Set(data.map(update => update.user_id).filter(Boolean))];
      console.log('📝 User IDs from updates:', userIds);
      ...
      if (userIds.length > 0) {
        ...
        if (!userError && users) {
          ...
          data = data.map(update => ({
            ...update,
            posted_by_user: userMap[update.user_id] || null
          }));
        } else if (userError) {
  ...
  async createIncidentUpdate(updateData) {
    // Use admin client to bypass RLS for incident update creation
    const client = this.adminClient || this.client;
    console.log('💾 Creating incident update:', updateData);
    
    // First try with visible_to_public if it's provided
    let insertData = { ...updateData };
    let shouldRetryWithoutVisibleToPublic = false;
    ...
    // If error mentions visible_to_public column not found, retry without it
    if (error && error.message?.includes('visible_to_public')) {
      console.log('⚠️ visible_to_public column not found, retrying without it');
      shouldRetryWithoutVisibleToPublic = true;
      
      const { visible_to_public, ...dataWithoutVisibleToPublic } = insertData;
      const { data: retryData, error: retryError } = await client
        .from('incident_updates')
        .insert(dataWithoutVisibleToPublic)
        .select()
        .single();
      ...
      // Add the flag back to the returned data for UI consistency
      return {
        ...retryData,
        visible_to_public: updateData.visible_to_public || false
      };
    }
  ...
  async updateMonitoringCheck(checkId, updateData) {
    try {
      ...
      if (updateData.http_method !== undefined) dbData.method = updateData.http_method;
      if (updateData.headers !== undefined) dbData.headers = updateData.headers;
      if (updateData.http_headers !== undefined) dbData.headers = updateData.http_headers;
      ...
      if (updateData.expected_status_codes !== undefined && updateData.expected_status_codes[0] !== undefined)
        dbData.expected_status_code = updateData.expected_status_codes[0];
      if (updateData.timeout !== undefined)
        dbData.timeout_seconds = updateData.timeout;
      ...
      if (updateData.check_interval !== undefined)
        dbData.check_interval_seconds = updateData.check_interval * 60; // Convert minutes to seconds
      ...
      if (updateData.failure_threshold_minutes !== undefined)
        dbData.failure_threshold_minutes = updateData.failure_threshold_minutes;
      ...
      if (updateData.linked_service_id !== undefined)
        dbData.linked_service_id = updateData.linked_service_id;
      
      // Skip fields that don't exist in the monitoring_checks table
      // These fields are from the UI but don't exist in the database schema
      console.log('⚠️ Skipping non-existent columns: update_service_status, service_failure_status, service_recovery_status');

      // Incident creation fields
      if (updateData.auto_create_incidents !== undefined)
        dbData.auto_create_incidents = updateData.auto_create_incidents;
      if (updateData.incident_severity !== undefined)
        dbData.incident_severity = updateData.incident_severity;
      if (updateData.incident_threshold_minutes !== undefined)
        dbData.incident_threshold_minutes = updateData.incident_threshold_minutes;
      if (updateData.incident_title_template !== undefined)
        dbData.incident_title_template = updateData.incident_title_template;
      if (updateData.incident_description_template !== undefined)
        dbData.incident_description_template = updateData.incident_description_template;
      if (updateData.auto_resolve_incidents !== undefined)
        dbData.auto_resolve_incidents = updateData.auto_resolve_incidents;
      if (updateData.assigned_on_call_schedule_id !== undefined)
        dbData.assigned_on_call_schedule_id = updateData.assigned_on_call_schedule_id;
      if (updateData.assigned_escalation_policy_id !== undefined)
        dbData.assigned_escalation_policy_id = updateData.assigned_escalation_policy_id;

      // Skip status_page_config - this field doesn't exist in monitoring_checks table
      if (updateData.status_page_config !== undefined) {
        console.log('⚠️ Skipping non-existent column: status_page_config');
        // Note: status_page_config is stored elsewhere, not in monitoring_checks table
      }

      console.log(`📝 Prepared database update data:`, dbData);
      console.log(`📝 Update data keys:`, Object.keys(dbData));
      console.log(`📝 Update data JSON:`, JSON.stringify(dbData, null, 2));

      // Try with just a minimal update first to test connectivity
      ...
      if (updateError) {
        ...
        console.log('🔍 Attempting to identify problematic field...');
        for (const [key, value] of Object.entries(dbData)) {
          if (key !== 'updated_at') {
            try {
              const testUpdate = { updated_at: new Date().toISOString(), [key]: value };
              const { error: fieldError } = await this.client
                .from('monitoring_checks')
                .update(testUpdate)
                .eq('id', checkId);
                
              if (fieldError) {
                console.error(`❌ Field '${key}' caused error:`, fieldError.message);
              } else {
                console.log(`✅ Field '${key}' updated successfully`);
              }
            } catch (fieldErr) {
              console.error(`❌ Field '${key}' caused exception:`, fieldErr.message);
            }
          }
        }
      ...
      if (selectError) {
        console.error(`❌ Error fetching updated check:`, selectError);
        throw selectError;
      }
  ...
  async getTeamGroup(teamId) {
    // Use admin client to bypass RLS for team fetching, consistent with getTeamGroups
    const client = this.adminClient || this.client;
    const { data, error } = await client
      .from('team_groups')
      .select('*')
  ...
  async updateTeamGroup(teamId, updateData) {
    // Use admin client to bypass RLS for team operations, consistent with other team methods
    const client = this.adminClient || this.client;
    const { data, error } = await client
      .from('team_groups')
      .update({
  ...
  async deleteTeamGroup(teamId) {
    // Use admin client to bypass RLS for team deletion, consistent with getTeamGroup
    const client = this.adminClient || this.client;
  ...
  async getTeamMemberships(teamId) {
    // Use admin client to bypass RLS for team member operations
    const client = this.adminClient || this.client;
    const { data, error } = await client
  ...
  async getOrganizationMembers(organizationId) {
    const { data, error } = await this.client
      ...
      .select(
        `
        *,
        users!organization_members_user_id_fkey(id, name, email, avatar_url, phone_number, incident_role, is_on_call)
      `
      )
      .eq('organization_id', organizationId)
      .eq('is_active', true)
  ...
  async searchUsers(organizationId, searchTerm = '') {
    let query = this.client
      ...
      .select(
        `
        *,
        users(id, name, email, avatar_url, phone_number, incident_role, is_on_call)
      `
      )
      .eq('organization_id', organizationId)
      .eq('is_active', true)
  ...
  // Get recent updates for dashboard feed
  async getRecentUpdates(organizationId, limit = 20) {
    const client = this.adminClient || this.client;
    const updates = [];

    try {
      // Get recent incident updates
      const { data: incidentUpdates, error: incidentError } = await client
        .from('incident_updates')
        .select(`
          *,
          incidents!inner(id, title, severity, status, organization_id)
        `)
        .eq('incidents.organization_id', organizationId)
        .order('created_at', { ascending: false })
        .limit(Math.floor(limit / 2));

      if (!incidentError && incidentUpdates) {
        updates.push(...incidentUpdates.map(update => ({
          id: `incident_update_${update.id}`,
          type: 'incident_update',
          title: `Incident Update: ${update.incidents.title}`,
          message: update.message,
          status: update.status,
          severity: update.incidents.severity,
          created_at: update.created_at,
          incident_id: update.incident_id,
          incident_title: update.incidents.title,
          incident_severity: update.incidents.severity,
      ...
      const { data: statusUpdates, error: statusError } = await client
        .from('status_updates')
        .select(`
          *,
          status_pages!inner(id, name, organization_id)
        `)
      ...
      if (!statusError && statusUpdates) {
        updates.push(...statusUpdates.map(update => ({
          id: `status_update_${update.id}`,
          type: 'status_update',
          title: `Status Update: ${update.status_pages.name}`,
          message: update.message,
          status: update.status,
          created_at: update.created_at,
          status_page_id: update.status_page_id,
          status_page_name: update.status_pages.name,
          posted_by: update.created_by,
          user_id: update.created_by,
        })));
      }

      // Sort all updates by creation time and limit
      updates.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
      const limitedUpdates = updates.slice(0, limit);

      // Get user information for all updates
      const userIds = [...new Set(limitedUpdates.map(update => update.user_id).filter(Boolean))];
      ...
      if (userIds.length > 0) {
        ...
        if (!userError && users) {
          ...
          limitedUpdates.forEach(update => {
            update.posted_by_user = userMap[update.user_id] || null;
          });
        }
      }
  ...
  async createIncidentEscalation(escalationData) {
    try {
      ...
      console.error('Error creating incident escalation:', error);
      throw error;
    }
  }

  async getActiveIncidentEscalations(incidentId) {
    try {
      const { data, error } = await supabase
        .from('incident_escalations')
        .select('*')
      ...
      console.error('Error getting active incident escalations:', error);
      throw error;
    }
  }

  async updateIncidentEscalation(escalationId, updateData) {
    try {
      const { data, error } = await supabase
        .from('incident_escalations')
        .update(updateData)
      ...
      console.error('Error updating incident escalation:', error);
      throw error;
    }
  }

  async getTimedOutEscalations() {
    try {
      ...
      console.error('Error getting timed out escalations:', error);
      throw error;
    }
  }

  async getAllIncidentEscalations(limit = 50) {
    try {
      const { data, error } = await supabase
        .from('incident_escalations')
        .select('*')
      ...
      console.error('Error getting all incident escalations:', error);
      throw error;
    }
  }
}
