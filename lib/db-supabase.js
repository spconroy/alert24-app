...
export class SupabaseClient {
  ...
  async getMonitoringChecks(userId, filters = {}) {
    try {
      ...
      if (false && filters.organization_id) {
        // Simplified filter - just use status_pages join for now
        query = query.eq(
      ...
      console.log(
        '🔍 Raw monitoring check data from DB:',
        data?.length || 0,
        'records'
      );
      console.log(
        '🔍 Sample record:',
        data?.[0] ? { name: data[0].name, id: data[0].id } : 'none'
      );
        ...
        .map(service => {
          try {
            ...
            if (checkData.type !== 'monitoring_check') {
              console.log('🔍 Skipping non-monitoring service:', service.name);
              return null;
            }
