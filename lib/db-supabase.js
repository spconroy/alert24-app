...
export class SupabaseClient {
      ...
      return (data || []).map(check => {
        ...
        return {
          ...
          current_status:
            ...
            (check.status === 'active' ? 'up' : 'inactive'),
  ...
  async createMonitoringCheck(checkData) {
    try {
      ...
      const dbData = {
        ...
        timeout_seconds: checkData.timeout || 30,
        check_interval_seconds: checkData.check_interval
          ? checkData.check_interval * 60
          : 300, // Convert minutes to seconds
      ...
      // Try with regular client first (which should work with proper RLS)
      ...
      // If RLS violation and we have admin client, try with admin client
      if (error && error.code === '42501' && this.adminClient) {
  ...
  async getServiceUptimeStats(serviceId, days = 30) {
    try {
      ...
      // Get all status history for this service in the time period
