  ...
  if (providerConfig.id === 'azure') {
    ...
    try {
      ...
      if (componentsResponse.ok) {
        ...
        console.log(
          `Found ${componentsData.components?.length || 0} components`
        );

        const serviceComponent = findCloudflareServiceComponent(
        ...
        if (serviceComponent) {
          ...
          console.log(
            `Found service component ${serviceComponent.name} with status: ${serviceStatus}`
          );
        } else {
          console.log(
            `No matching component found for service: ${serviceConfig.id}`
          );
        }
      } else {
      ...
      console.warn(
        'Failed to fetch Cloudflare components, using overall status:',
        error.message
      );
    }
    ...
    if (serviceStatus) {
      ...
      rawStatus = status;
      console.log(
        `Using overall status: ${status} (indicator: ${jsonData.status.indicator}, description: ${jsonData.status.description})`
      );
    } else if (jsonData.page && jsonData.page.status_indicator) {
      status = jsonData.page.status_indicator;
      rawStatus = status;
...
function findCloudflareServiceComponent(components, serviceConfig) {
  ...
  const possibleNames = serviceNameMappings[serviceConfig.id] || [
    serviceConfig.name,
  ];
  console.log(
    `Looking for Cloudflare service ${serviceConfig.id} with possible names:`,
    possibleNames
  );
  ...
  for (const component of components) {
    if (component.name) {
      ...
      for (const possibleName of possibleNames) {
        if (
          componentName.includes(possibleName.toLowerCase()) ||
          possibleName.toLowerCase().includes(componentName)
        ) {
          console.log(
            `Found matching component: ${component.name} (status: ${component.status})`
          );
          return component;
        }
      }
...
function parseStatusFromHtml(html, provider, service) {
  ...
  // Special handling for Google Cloud Platform - look for specific service
  if (provider === 'gcp') {
    // Map service codes to their display names in GCP status page
    // Note: Names must match exactly as they appear on https://status.cloud.google.com/
    const gcpServiceMap = {
      ...
      'gcp-kubernetes-engine': 'Google Kubernetes Engine',
      'gcp-cloud-run': 'Cloud Run',
      'gcp-bigquery': 'BigQuery',
      'gcp-cloud-datastore': 'Google Cloud Datastore',
      'gcp-cloud-firestore': 'Cloud Firestore',
      'gcp-cloud-functions': 'Google Cloud Functions', // Correct name from status page
    };

    const serviceName = gcpServiceMap[service] || service;
    console.log(`Looking for GCP service: "${serviceName}"`);

    // Find the specific service row in the HTML table
    // GCP status page uses table format: <th class="j2GwVIZkdLB__product">Service Name</th>
    const serviceRegex = new RegExp(
      `<th class="j2GwVIZkdLB__product">${serviceName}</th>`,
      'i'
    );
    const serviceMatch = html.match(serviceRegex);

    if (!serviceMatch) {
      console.log(`❌ Service "${serviceName}" not found in GCP status page`);
      console.log(
        `💡 Available services include: Compute Engine, Cloud Storage, App Engine, Cloud SQL, etc.`
      );
      console.log(
        `🔍 Check https://status.cloud.google.com/ for the complete list`
      );
      // Fall back to general counting for unknown services
      const greenIndicators = (html.match(/psd__available/g) || []).length;
      const redIndicators = (
        html.match(/psd__disruption|psd__outage|psd__degraded/g) || []
      ).length;
      const yellowIndicators = (
        html.match(/psd__information|psd__investigating/g) || []
      ).length;

      console.log(
        `GCP fallback: ${greenIndicators} green, ${redIndicators} red, ${yellowIndicators} yellow`
      );

      if (redIndicators > 0) return 'major_outage';
      if (yellowIndicators > 0) return 'degraded_performance';
      if (greenIndicators > 0) return 'operational';
      return 'unknown';
    }

    // Extract the table row for this specific service
    const serviceIndex = html.indexOf(serviceMatch[0]);
    const rowStart = html.lastIndexOf('<tr>', serviceIndex);
    const rowEnd = html.indexOf('</tr>', serviceIndex) + 5;
    const serviceRow = html.substring(rowStart, rowEnd);

    console.log(`Found service row for "${serviceName}"`);

    // Count status indicators in this specific row
    const rowGreenCount = (serviceRow.match(/psd__available/g) || []).length;
    const rowRedCount = (
      serviceRow.match(/psd__disruption|psd__outage|psd__degraded/g) || []
    ).length;
    const rowYellowCount = (
      serviceRow.match(/psd__information|psd__investigating/g) || []
    ).length;

    console.log(
      `Service "${serviceName}" status: ${rowGreenCount} green, ${rowRedCount} red, ${rowYellowCount} yellow in service row`
    );

    // Determine status based on the specific service row
    if (rowRedCount > 0) {
      return 'major_outage';
    } else if (rowYellowCount > 0) {
      return 'degraded_performance';
    } else if (rowGreenCount > 0) {
      return 'operational';
    } else {
      // No status indicators found for this service in any region
      return 'no_data';
    }
  }

  // Special handling for providers with dynamic content
  if (provider === 'stripe') {
    // Stripe uses React and loads content dynamically
    // If we get the loading page, assume operational unless there are incidents
    if (lowerHtml.includes('loading...') && !lowerHtml.includes('incident')) {
      console.log(
        'Stripe page is loading dynamically, assuming operational status'
      );
      return 'operational';
    }
  }
