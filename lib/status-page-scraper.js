...
function parseStatusFromHtml(html, provider, service) {
  ...
  // Special handling for Google Cloud Platform
  if (provider === 'gcp') {
    // GCP shows green checkmarks (psd__available) for operational services
    // Count indicators with actual status vs total rows to get better ratio
    ...
    const totalStatusIndicators =
      greenIndicators + redIndicators + yellowIndicators;

    console.log(
      `GCP status: ${greenIndicators} green, ${redIndicators} red, ${yellowIndicators} yellow out of ${totalStatusIndicators} total status indicators`
    );

    // If we have any green indicators and no significant outages, consider operational
    if (greenIndicators > 0) {
      if (redIndicators === 0 && yellowIndicators < greenIndicators * 0.1) {
        return 'operational';
      } else if (redIndicators < greenIndicators * 0.1) {
        return 'degraded_performance';
      } else {
        return 'major_outage';
      }
    } else {
      // No green indicators found, check for any incidents
      if (lowerHtml.includes('incident') || lowerHtml.includes('outage')) {
        return 'major_outage';
      } else {
        // Assume operational if no explicit indicators of problems
        return 'operational';
      }
    }
  }
