...
async function createMonitoringCheck(serviceId, url, apiUrl, serviceName) {
  try {
    ...
    if (error) {
      console.error(`    ⚠️  Failed to create monitoring check: ${error.message}`);
      return null;
    }
  ...
  for (const item of compatible) {
    ...
    try {
      ...
      if (existing && existing.length > 0) {
        console.log(`   ⏭️  Skipped - similar service already exists: ${existing[0].name}`);
        skipped++;
        continue;
      }
  ...
  try {
    ...
    for (const item of compatible) {
      ...
      content = content.replace(urlRegex, (match, before, urlMatch, after) => {
        ...
        marked++;
        return before.replace(/→✅/, '→🟢 [IMPORTED]').replace(/→/, '→🟢 [IMPORTED]') + urlMatch + after;
      });
    }
...
function generateSummary(stats, organizationName) {
  ...
  if (stats.imported > 0) {
    console.log(`\n🚀 What's Next:`);
    console.log(`• ${stats.imported} new external services are now being monitored`);
  ...
  const report = {
    ...
    imported_services: compatibilityReport.compatible.slice(0, stats.imported).map(item => ({
