...
export async function GET(req, { params }) {
  try {
    ...
    `;
    const { rows: memberRows } = await pool.query(membershipQuery, [
...
export async function PUT(req, { params }) {
  try {
    ...
    const { name, description, status, sort_order, auto_recovery } = body;
    ...
    const updateQuery = `
      ...
      SET 
        ...
        auto_recovery = COALESCE($6, auto_recovery),
    ...
    const { rows: updatedServices } = await pool.query(updateQuery, [
      ...
      status || null,
      sort_order || null,
      auto_recovery !== undefined ? auto_recovery : null,
    ]);
...
export async function DELETE(req, { params }) {
  try {
    ...
    const { rows: deletedServices } = await pool.query(deleteQuery, [
      serviceId,
    ]);
