...
export async function POST(request) {
  try {
    const { organizationId, dateRange, services, format } =
      await request.json();
...
function generateCSV(data) {
  ...
  csv += 'Service Summary\\n';
  csv +=
    'Service Name,Total Checks,Successful Checks,Uptime %,Avg Response Time\\n';

  services.forEach(service => {
    const serviceStats = statistics.filter(
      stat => stat.service_id === service.id
    );
    const totalChecks = serviceStats.reduce(
      (sum, stat) => sum + (stat.total_checks || 0),
      0
    );
    const successfulChecks = serviceStats.reduce(
    ...
    const avgResponseTime =
      serviceStats.length > 0
        ? Math.round(
            serviceStats.reduce(
              (sum, stat) => sum + (stat.avg_response_time || 0),
              0
            ) / serviceStats.length
          )
        : 0;
