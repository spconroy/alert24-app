  ...
  try {
    ...
    const processedIncidents = (incidents || []).map(incident => {
      ...
      const acknowledgmentTime = acknowledged
        ? Math.round((acknowledged - created) / (1000 * 60))
        : 0; // minutes
    ...
    const openIncidents = processedIncidents.filter(
      i => i.status !== 'resolved'
    ).length;
    ...
    const mttr =
      resolvedIncidents.length > 0
        ? Math.round(
            resolvedIncidents.reduce((sum, i) => sum + i.duration, 0) /
              resolvedIncidents.length
          )
        : 0;

    const acknowledgedIncidents = processedIncidents.filter(
      i => i.acknowledged_at
    );
    const mtta =
      acknowledgedIncidents.length > 0
        ? Math.round(
            acknowledgedIncidents.reduce(
              (sum, i) => sum + i.acknowledgmentTime,
              0
    ...
    const severityCounts = {
      critical: processedIncidents.filter(i => i.severity === 'critical')
        .length,
