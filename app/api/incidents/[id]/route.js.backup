...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    const canEdit =
      currentIncident.incident_role === 'admin' ||
    ...
    if (!canEdit) {
      return new Response(
        JSON.stringify({
          error: 'Insufficient permissions to edit this incident',
        }),
        { status: 403 }
      );
    }
    ...
    if (
      escalation_policy_id &&
      escalation_policy_id !== currentIncident.escalation_policy_id
    ) {
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          error: 'Only admins and managers can delete incidents',
        }),
        { status: 403 }
      );
    }
    ...
    try {
      ...
      await pool.query(
        'DELETE FROM public.incident_updates WHERE incident_id = $1',
        [id]
      );

      // Delete incident escalations
      await pool.query(
        'DELETE FROM public.incident_escalations WHERE incident_id = $1',
        [id]
      );

      // Delete the incident
      const deleteRes = await pool.query(
        'DELETE FROM public.incidents WHERE id = $1 RETURNING title',
        [id]
      );
      ...
      console.log(
        `Incident "${deleteRes.rows[0].title}" (${id}) deleted by user ${session.user.email}`
      );
