...
export async function POST(req, { params }) {
  ...
  try {
    ...
    const canPost = incident.incident_role === 'admin' || 
    ...
    if (!canPost) {
      return new Response(JSON.stringify({ error: 'Insufficient permissions to post updates to this incident' }), { status: 403 });
    }
    ...
    const updateRes = await pool.query(`
      ...
      RETURNING *
    `, [
      incidentId, message, finalStatus, update_type, user.id,
      visible_to_subscribers, notify_subscribers
    ]);
    ...
    const fullUpdateRes = await pool.query(`
      SELECT iu.*,
             u.name as posted_by_name, u.email as posted_by_email
      FROM public.incident_updates iu
      JOIN public.users u ON iu.posted_by = u.id
