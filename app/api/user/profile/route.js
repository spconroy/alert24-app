...
export async function PUT(req) {
  ...
  try {
    ...
    // Get current user
    const currentUserRes = await pool.query('SELECT id FROM public.users WHERE email = $1', [session.user.email]);
    ...
    if (email !== session.user.email) {
      const existingUserRes = await pool.query('SELECT id FROM public.users WHERE email = $1 AND id != $2', [email, userId]);
      if (existingUserRes.rows.length > 0) {
        return new Response(JSON.stringify({ error: 'Email address is already in use by another account' }), { status: 409 });
      }
    }
    ...
    if (typeof parsedNotificationPreferences === 'string') {
      try {
        parsedNotificationPreferences = JSON.parse(parsedNotificationPreferences);
      } catch (e) {
