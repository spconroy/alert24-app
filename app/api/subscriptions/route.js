...
export async function POST(req) {
  try {
    ...
    if (!status_page_id || !email) {
      return new Response(JSON.stringify({ error: 'Missing status_page_id or email' }), { status: 400 });
    }
    ...
    `;
    const { rows: statusPages } = await pool.query(statusPageQuery, [status_page_id]);
    ...
    if (!statusPage.is_public) {
      return new Response(JSON.stringify({ error: 'Cannot subscribe to private status page' }), { status: 403 });
    }
    ...
    `;
    const { rows: existingSubscriptions } = await pool.query(existingSubscriptionQuery, [status_page_id, email]);
    ...
    const { rows: newSubscriptions } = await pool.query(insertSubscriptionQuery, [status_page_id, email]);
...
export async function DELETE(req) {
  try {
    ...
    const { rows: unsubscribed } = await pool.query(unsubscribeQuery, queryParams);
