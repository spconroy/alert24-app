...
export async function POST(req) {
  try {
    ...
    // Insert status page
    console.log('Attempting to insert status page:', { org_id, name, slug, description, is_public });
    const insertQuery = `
      INSERT INTO public.status_pages (organization_id, name, slug, description, is_public)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `;
    const { rows } = await pool.query(insertQuery, [org_id, name, slug, description || null, is_public ?? true]);
    console.log('Status page created successfully:', rows[0]);
    return new Response(JSON.stringify({ statusPage: rows[0] }), { status: 201 });
  } catch (err) {
