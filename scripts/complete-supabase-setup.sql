-- ============================================================================
-- ALERT24 COMPLETE SUPABASE SETUP SCRIPT
-- ============================================================================
...
CREATE INDEX idx_uptime_records_service_id ON public.service_uptime_records(service_id);
CREATE INDEX idx_uptime_records_checked_at ON public.service_uptime_records(checked_at);
CREATE INDEX idx_uptime_records_status ON public.service_uptime_records(status);

-- Activity History
CREATE INDEX idx_activity_history_org_id ON public.activity_history(organization_id);
CREATE INDEX idx_activity_history_actor_id ON public.activity_history(actor_id);
...
-- Service Monitoring Configuration
INSERT INTO public.service_monitoring (service_id, organization_id, enabled, check_type, endpoint_url, expected_status_code, check_interval_minutes) VALUES 
('s1000000-1000-4000-8000-s10000000001', 'a0000000-a000-4000-8000-a00000000001', true, 'http', 'https://alert24.com', 200, 5),
('s2000000-2000-4000-8000-s20000000002', 'a0000000-a000-4000-8000-a00000000001', true, 'http', 'https://api.alert24.com/health', 200, 2),
('s3000000-3000-4000-8000-s30000000003', 'a0000000-a000-4000-8000-a00000000001', true, 'http', 'https://app.alert24.com/health', 200, 5),
('s5000000-5000-4000-8000-s50000000005', 'b0000000-b000-4000-8000-b00000000002', true, 'http', 'https://shop.acme.com', 200, 3),
('s6000000-6000-4000-8000-s60000000006', 'b0000000-b000-4000-8000-b00000000002', true, 'http', 'https://payments.acme.com/ping', 200, 1),
('s8000000-8000-4000-8000-s80000000008', 'c0000000-c000-4000-8000-c00000000003', true, 'http', 'https://startup.example.com', 200, 10);
...
-- Incident Updates
INSERT INTO public.incident_updates (incident_id, organization_id, status, message, is_public, created_by, created_at) VALUES 
('i1000000-1000-4000-8000-i10000000001', 'b0000000-b000-4000-8000-b00000000002', 'investigating', 'We are investigating reports of slow payment processing. Our team is looking into the issue.', true, '20000000-2000-4000-8000-200000000002', NOW() - INTERVAL '2 hours'),
('i1000000-1000-4000-8000-i10000000001', 'b0000000-b000-4000-8000-b00000000002', 'identified', 'We have identified the root cause as increased database load. Implementing additional capacity.', true, '20000000-2000-4000-8000-200000000002', NOW() - INTERVAL '1 hour'),
('i2000000-2000-4000-8000-i20000000002', 'c0000000-c000-4000-8000-c00000000003', 'monitoring', 'Database upgrade completed successfully. Monitoring performance and stability.', true, '30000000-3000-4000-8000-300000000003', NOW() - INTERVAL '15 minutes');

-- Sample Status Updates
INSERT INTO public.status_updates (status_page_id, organization_id, incident_id, title, message, status, is_published, published_at, created_by) VALUES 
('p2000000-2000-4000-8000-p20000000002', 'b0000000-b000-4000-8000-b00000000002', 'i1000000-1000-4000-8000-i10000000001', 'Payment Processing Issues', 'We are currently experiencing slow response times with our payment gateway. Our engineering team is working to resolve this issue. We will provide updates as we have more information.', 'identified', true, NOW() - INTERVAL '1 hour', '20000000-2000-4000-8000-200000000002'),
('p3000000-3000-4000-8000-p30000000003', 'c0000000-c000-4000-8000-c00000000003', 'i2000000-2000-4000-8000-i20000000002', 'Scheduled Database Maintenance', 'We are performing scheduled maintenance on our database infrastructure. Some services may be temporarily unavailable. Expected completion: 1 hour.', 'monitoring', true, NOW() - INTERVAL '30 minutes', '30000000-3000-4000-8000-300000000003');
...
('p1000000-1000-4000-8000-p10000000001', 'notifications@company.com', true, false, NOW() - INTERVAL '3 days'),
...
-- Sample Uptime Records (last 24 hours of data)
INSERT INTO public.service_uptime_records (service_id, organization_id, status, response_time_ms, status_code, checked_at)
SELECT 
    's1000000-1000-4000-8000-s10000000001',
    'a0000000-a000-4000-8000-a00000000001',
    CASE 
        WHEN random() > 0.05 THEN 'up'::VARCHAR(50)
        ELSE 'down'::VARCHAR(50)
    END,
    (150 + random() * 300)::INTEGER, -- Response time between 150-450ms
    200,
    NOW() - (generate_series(1, 288) * INTERVAL '5 minutes') -- Every 5 minutes for 24 hours
FROM generate_series(1, 288);

-- Sample SLA Records
INSERT INTO public.sla_records (service_id, organization_id, period_start, period_end, total_checks, successful_checks, uptime_percentage, average_response_time_ms) VALUES 
('s1000000-1000-4000-8000-s10000000001', 'a0000000-a000-4000-8000-a00000000001', NOW() - INTERVAL '30 days', NOW(), 8640, 8590, 99.42, 275.50),
('s2000000-2000-4000-8000-s20000000002', 'a0000000-a000-4000-8000-a00000000001', NOW() - INTERVAL '30 days', NOW(), 21600, 21580, 99.91, 125.30),
('s5000000-5000-4000-8000-s50000000005', 'b0000000-b000-4000-8000-b00000000002', NOW() - INTERVAL '30 days', NOW(), 14400, 14350, 99.65, 200.75);

-- Sample Activity History
INSERT INTO public.activity_history (organization_id, actor_id, action, entity_type, entity_id, entity_name) VALUES 
('a0000000-a000-4000-8000-a00000000001', '10000000-1000-4000-8000-100000000001', 'created', 'service', 's1000000-1000-4000-8000-s10000000001', 'Main Website'),
('a0000000-a000-4000-8000-a00000000001', '10000000-1000-4000-8000-100000000001', 'created', 'status_page', 'p1000000-1000-4000-8000-p10000000001', 'Alert24 System Status'),
('b0000000-b000-4000-8000-b00000000002', '20000000-2000-4000-8000-200000000002', 'created', 'incident', 'i1000000-1000-4000-8000-i10000000001', 'Payment Gateway Slowness'),
('c0000000-c000-4000-8000-c00000000003', '30000000-3000-4000-8000-300000000003', 'updated', 'service', 's9000000-9000-4000-8000-s90000000009', 'Beta App');
...
COMMENT ON TABLE public.organizations IS 'Core multi-tenant entity representing customer organizations';
COMMENT ON TABLE public.users IS 'User accounts with Google OAuth integration';
COMMENT ON TABLE public.services IS 'Services and endpoints being monitored';
COMMENT ON TABLE public.incidents IS 'Service incidents and outages';
COMMENT ON TABLE public.status_pages IS 'Public status pages for organizations';
COMMENT ON TABLE public.notifications IS 'In-app and email notifications';
COMMENT ON TABLE public.activity_history IS 'Audit log for all user and system actions';
