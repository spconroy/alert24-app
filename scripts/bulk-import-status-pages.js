...
function categorizeService(url, name) {
  ...
  if (
    ...
    lowerUrl.includes('gcp') ||
    lowerUrl.includes('cloud') ||
    lowerUrl.includes('oracle') ||
    lowerUrl.includes('digitalocean') ||
    lowerUrl.includes('linode') ||
    lowerUrl.includes('vultr') ||
    lowerUrl.includes('scaleway') ||
    lowerUrl.includes('upcloud') ||
    lowerUrl.includes('kinsta') ||
    lowerUrl.includes('cloudways') ||
    lowerUrl.includes('hetzner') ||
    lowerUrl.includes('ovh')
  ) {
  ...
  // Communication
  if (
    lowerUrl.includes('sendgrid') ||
    lowerUrl.includes('mailgun') ||
    lowerUrl.includes('twilio') ||
    lowerUrl.includes('zoom') ||
    lowerUrl.includes('slack') ||
    lowerUrl.includes('webex')
  ) {
  ...
  if (
    ...
    lowerUrl.includes('plaid') ||
    lowerUrl.includes('coinbase') ||
    lowerUrl.includes('robinhood') ||
    lowerUrl.includes('klarna') ||
    lowerUrl.includes('adyen')
  ) {
  ...
  if (
    ...
    lowerUrl.includes('bigcommerce') ||
    lowerUrl.includes('magento')
  ) {
  ...
  if (
    ...
    lowerUrl.includes('monday') ||
    lowerUrl.includes('trello') ||
    lowerUrl.includes('airtable') ||
    lowerUrl.includes('miro') ||
    lowerUrl.includes('taskade') ||
    lowerUrl.includes('hubspot') ||
    lowerUrl.includes('netsuite')
  ) {
  ...
  // Security
  if (
    lowerUrl.includes('duo') ||
    lowerUrl.includes('auth0') ||
    lowerUrl.includes('okta') ||
    lowerUrl.includes('sumsub') ||
    lowerUrl.includes('tailscale')
  ) {
  ...
  if (
    ...
    lowerUrl.includes('amplitude') ||
    lowerUrl.includes('datadog') ||
    lowerUrl.includes('appsflyer') ||
    lowerUrl.includes('grammarly')
  ) {
    return 'Analytics & AI';
  }
...
async function updateStatusPageList() {
  ...
  try {
    ...
    fs.writeFileSync(
      'status_page_list_original.txt',
      fs.readFileSync('status_page_list.txt', 'utf8')
    );
...
async function generateImportReport(stats) {
  ...
  fs.writeFileSync(
    'status-pages-import-report.json',
...
async function main() {
  ...
  try {
    ...
    if (stats.imported > 0) {
      ...
      console.log(
        '📄 Check status-pages-import-report.json for detailed information'
      );
      console.log(
        '📝 status_page_list.txt has been updated with import status'
      );
    }
  } catch (error) {
