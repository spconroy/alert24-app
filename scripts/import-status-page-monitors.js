...
async function createStatusPageMonitors(organizationId) {
  ...
  for (const item of compatible) {
    ...
    try {
      ...
      if (checkError) {
        console.error(
          `   ❌ Error checking existing monitors: ${checkError.message}`
        );
        errors++;
        continue;
      }
      ...
      if (existing && existing.length > 0) {
        console.log(
          `   ⏭️  Skipped - monitoring check already exists: ${existing[0].name}`
        );
        skipped++;
        continue;
      }
      ...
      console.log(
        `   ✅ Created monitoring check: ${monitor.name} (ID: ${monitor.id})`
      );
      imported++;
    } catch (error) {
...
function generateSummary(stats, organizationName) {
  ...
  if (stats.imported > 0) {
    ...
    console.log(
      `• ${stats.imported} external status page APIs are now being monitored`
    );
    console.log(
      `• Checks run every 5 minutes to detect when services report issues`
    );
    console.log(
      `• You'll get alerts if any monitored status pages go down or report outages`
    );
    console.log(
      `• This helps you stay ahead of issues with services your infrastructure depends on`
    );
  }
  ...
  const report = {
    ...
    monitored_services: compatibilityReport.compatible
  ...
  fs.writeFileSync(
    'status-page-monitoring-report.json',
    JSON.stringify(report, null, 2)
  );
  console.log(
    `• status-page-monitoring-report.json - Detailed monitoring report`
  );

  console.log(`\n💡 Next Steps:`);
  console.log(
    `• Check your Alert24 dashboard to see the new monitoring checks`
  );
  console.log(
    `• Configure alert rules if you want notifications for status page issues`
  );
